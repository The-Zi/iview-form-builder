 ...
 * @Last Modified time: 2018-05-30 17:56:34
 */
  ...
  <Row class="form-grid-wrap">
              ...
              <div class="form-modules-item" v-for="(modules, mIndex) in col" :key="'form-modules-' + mIndex">
                ...
                <FormItem class="form-grid-item" v-if="modules.type === '时间日期范围'"
                 :required="modules.require" :label="modules.label">
                  ...
                  <!-- 组件 -->
                  <DatePicker type="datetimerange" format="yyyy年MM月dd日 HH时mm分"
                  placeholder="请选择时间日期范围" v-model="modules.value" @on-ok="say(modules.value)"></DatePicker>
                </FormItem>
                ...
                <FormItem class="form-grid-item" v-if="modules.type === '时间日期'"
                 :required="modules.require" :label="modules.label">
                  ...
                  <!-- 组件 -->
                  <DatePicker type="datetime" format="yyyy年MM月dd日 HH时mm分" placeholder="请选择时间日期"
    ...
    <Modal v-model="modulesSetingModalToogle">
      ...
      <Form ref="configForm" :rules="ruleValidate" :model="moduleConfig" label-position="left"
       :label-width="100">
        <!-- 设为必填项 -->
        <FormItem v-show="moduleConfig.type !== '说明文本'" label="设为必填项：">
          <Checkbox v-model="moduleConfig.require"></Checkbox>
        </FormItem>

        <!-- 通用: 字段名 -->
        <FormItem label="字段名：" prop="name">
          <Input v-model="moduleConfig.name" placeholder="只能输入英文"
           @keyup.native="filterBlank"></Input>
        </FormItem>
        ...
        <!-- 文本输入框配置 -->
        <template v-if="moduleConfig.type === '文本输入框'">
          <FormItem label="默认值：">
        ...
        <!-- 数字输入框配置 -->
        <template v-if="moduleConfig.type === '数字输入框'">
          <FormItem label="默认值：">
        ...
        <!-- 单选配置 -->
        <template v-if="moduleConfig.type === '单选'">
          <FormItem label="默认值：">
             <Input v-model="moduleConfig.value" placeholder="默认值"></Input>
          </FormItem>
        ...
        <!-- 多选配置 -->
        <template v-if="moduleConfig.type === '多选'">
          <FormItem label="默认值：">
        ...
        <!-- 下拉选择配置 -->
        <template v-if="moduleConfig.type === '下拉选择'">
          <FormItem label="默认值：">
             <Input v-model="moduleConfig.value" placeholder="默认值"></Input>
          </FormItem>
        ...
        <!-- 多行文本配置 -->
        <template v-if="moduleConfig.type === '多行文本'">
          <FormItem label="默认值：">
        ...
        <!-- 说明文本 -->
        <template v-if="moduleConfig.type === '说明文本'">
          <FormItem label="默认值：">
            <Input v-model="moduleConfig.value" type="textarea" :rows="4"></Input>
          </FormItem>
        </template>
    ...
    methods: {
      ...
      drop:function(params, $event){
        ...
        switch (module) {
          ...
          case "文本输入框":
            tempItem.label = "文本输入框：";
            tempItem.placeholder = "文本输入框";
            break;

          case "数字输入框":
            tempItem.label = "数字输入框：";
            tempItem.placeholder = "只能输入数字";
            tempItem.value = null;
            tempItem.max = 20000;
            tempItem.min = 0;
            break;
      ...
      setModule: function (params) {
        ...
        // 设置组件特有配置
        if(module.type === "单选" || module.type === "多选" || module.type === "下拉选择") {
          this.moduleConfig.editList = module.itemList.join("##");
        }

        if (module.type === "数字输入框") {
