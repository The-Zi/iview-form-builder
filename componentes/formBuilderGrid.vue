                ...
                <div v-if="mode !== modeOption.element.type" >
                  <div v-for="(modules, mIndex) in col" :key="mIndex">
                    ...
                    :required="modules.require" :label="modules.label">
                        ...
                        @update="saveDetailElement(modules, $event)"
                        @delete="delDetailElement(modules, $event)">
  ...
  export default {
    ...
    methods: {
      ...
      drop: function (params, $event) {
        ...
        if (this.mode === this.modeOption.element.type) {
          if (tempItem.type !== this.element.detailed.type) {
            ...
            // 触发更新事件，以对象形式传递新添加的表单元素
            this.$emit("update", tempItem);
          }
        } else {
          // 添加新表单组件
          this.elementsData[params.rowIndex][params.colIndex].push(tempItem);
      ...
      countElementsNum: function (params) {
        if (this.elementsLength[params.type] >= 0) {
        ...
        return params.title + " " +
        (this.elementsLength[params.type] === 0 ? "" : this.elementsLength[params.type]) + "：";
      },
      ...
      saveModuleConfig: function () {
        ...
        this.$refs.configForm.validate((result)=>{
          if (result) {
            ...
            // 触发更新事件，以对象形式传递新添加的表单元素
            this.$emit("update", this.elementsData[row][col][m]);
      ...
      delModule: function (params) {
        ...
        // 触发更新事件，以对象形式传递新添加的表单元素
        this.$emit("delete", this.elementsData[row][col][m]);
      ...
      setDetailRenderData: function (params) {
        this.detailedData[params.id] = [[[]]];
      },

      // 保存 表单元素：明细 内的表单元素
      saveDetailElement: function (params, $event) {
      ...
      plusDetailElements: function (params) {
        ...
        for (let index = 0; index < params.childElements.length; index++) {
          ...
          newElement[params.id][0][0].push(temp);
        }

        // 更新 表单元素：明细 的渲染数据
        for (const key in this.detailedData) {
          if (this.detailedData.hasOwnProperty(key)) {
            this.detailedData[key][0][0][0] = 4444
          }
        }

        this.detailedData = Object.assign({}, this.detailedData, newElement);
      },
