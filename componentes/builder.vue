...
<template>
  <Row>
    ...
    <!-- 表单侧边栏 -->
    <Col span="8">
      <sidebar :formElementList="formElementList" @save="saveEvent({action:true})" @clear="clearEvent">
      </sidebar>
    </Col>
  </Row>
</template>
...
export default {
  ...
  methods: {
    ...
    saveEvent(params) {
      // 自定义事件：保存表单构建器数据
      this.$emit("saveEvent", {base: this.baseData, form: this.$refs.grid.getBuilderData()});
    }
  }
}
</script>
